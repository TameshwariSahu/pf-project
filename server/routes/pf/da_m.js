...
const { applyDASchema } = require("../../middleware/schemas");
const { getISTTime } = require('../../utils/time');

router.post("/apply-da", validate(applyDASchema), (req, res) => {
  ...
  db.query(
    ...
    (err) => {
      if (err) { console.log("INSERT DA ERROR:", err); return res.status(500).send("Server error ❌"); }
      db.query(
        `UPDATE pf_t p
        ...
        (err) => {
          if (err) { console.log("UPDATE DA ERROR:", err); return res.status(500).send("Server error ❌"); }
...
router.get("/get-all", (req, res) => {
  ...
  db.query(
    ...
    [category],
    (err, daRows) => {
      if (err) { console.log("GET-ALL ERROR:", err); return res.status(500).send("Server error ❌"); }
      const daMap = {};
